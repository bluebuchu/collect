...
class MockStorage {
    constructor() {
        ...
        this.sentenceLikes = new Map(); // "sentenceId-userId" => true
        this.communities = new Map();
        this.communityMembers = new Map(); // "communityId-userId" => member
        this.communitySentences = new Map(); // "communityId-sentenceId" => true
        this.books = new Map();
        this.booksByIsbn = new Map();
        this.nextUserId = 1;
    ...
    async createSentence(sentence, userId) {
        ...
        if (!user)
            throw new Error(`User not found with id: ${actualUserId}`);
    ...
    async getAllCommunitiesEnhanced(options) {
        ...
        // Filter by search
        if (options.search) {
            communities = communities.filter(c => c.name.toLowerCase().includes(options.search.toLowerCase()) ||
        ...
        switch (options.sort) {
            case 'members':
                ...
                break;
            case 'recent':
                sortedCommunities = enhancedCommunities.sort((a, b) => new Date(b.lastActivityAt || b.createdAt).getTime() -
    ...
    async getOrCreateBook(bookData) {
        ...
        const existingBook = Array.from(this.books.values()).find(b => b.title === bookData.title && b.author === bookData.author);
        if (existingBook) {
        ...
        const books = Array.from(bookStats.values())
            ...
            if (!cover && stats.book.title) {
                try {
                    ...
                    const searchUrl = `http://www.aladin.co.kr/ttb/api/ItemSearch.aspx?ttbkey=${TTBKey}&Query=${encodeURIComponent(stats.book.title)}&QueryType=Title&MaxResults=1&start=1&SearchTarget=Book&output=js&Version=20131101`;
