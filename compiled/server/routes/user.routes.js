...
router.get("/api/user/sentences", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
        const { search, sort = 'latest' } = req.query;
        let userSentences = await storage_1.storage.getUserSentences(userId);
        if (search) {
            const searchLower = search.toString().toLowerCase();
            userSentences = userSentences.filter(s => s.content.toLowerCase().includes(searchLower) ||
...
// Get user stats
router.get("/api/user/stats", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
...
router.get("/api/stats", async (req, res) => {
    try {
        const stats = await storage_1.storage.getOverallStats();
        res.json(stats);
    }
    catch (error) {
...
router.get("/api/recent-activity", async (req, res) => {
    try {
        ...
        const activities = await storage_1.storage.getRecentActivity(limit);
        res.json(activities);
    }
    catch (error) {
...
router.get("/api/contributors", async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 10;
        const contributors = await storage_1.storage.getTopContributors(limit);
