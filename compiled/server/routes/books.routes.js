...
router.get("/api/books/search", async (req, res) => {
    try {
        ...
        const searchUrl = `http://www.aladin.co.kr/ttb/api/ItemSearch.aspx?ttbkey=${TTBKey}&Query=${encodeURIComponent(query.toString())}&QueryType=Title&MaxResults=20&start=1&SearchTarget=Book&output=js&Version=20131101`;
        try {
            ...
            const books = await Promise.all(data.item.map(async (item) => {
                const bookData = {
            ...
            ].filter(book => book.title.toLowerCase().includes(query.toString().toLowerCase()) ||
